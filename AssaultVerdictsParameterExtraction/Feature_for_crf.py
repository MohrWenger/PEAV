...
# HEB_WORDS_TO_EXTRACT = ['עו*תרה*(ים)*(ות)*','ה*תובעת*','ביקשה*','ה*תביעה',"ה*סנגור","בי*קשה*","ערעור","ב\"כ"
#                         'מבחן','צבאי','בי*טחון','קבע','דורשת*','בימים',
#                         'בין','מתחם','יפחת','יעלה','נגזר','נדון','ה*צדדים',"\"","/",r"\\",":"
#                         ,'גוזרת*(ים)*(ות)*'
#                         ,"ריצוי בפועל","מה[ןם]","היתרה","מתו*כ[ןם]"
#                         '[1-9]+ *חודשי מאסר','[1-9]+[.]{4}שנות מאסר'
...
HEB_WORDS_TO_EXTRACT = ['בפועל','מתחם']
PENALTY_CANDIDATES = ['[1-9]+.*חו*דשי מאסר','[1-9]+.*שנו*ת מאסר']
...
def penalty_time_cadidate(sentence):
    for word in PENALTY_CANDIDATES:
        indices = [index.start() for index in re.finditer(word, sentence)]
        length = len(indices)
        list_of_indices.append([-1 if length == 0 else indices[0],
                                -1 if length == 0 else indices[0] / len(sentence),
